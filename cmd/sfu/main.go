...
import (
	...
	"github.com/doublemo/baa/cmd/sfu/server"
...
func ver() {
	fmt.Printf("sfu-server version %s commitid %s builddate %s\n", version, commitid, builddate)
	os.Exit(0)
}
...
func main() {
	...
	fs.StringVar(&fp, "c", "conf/sfu.conf", "Configuration file")
	fs.StringVar(&fp, "config", "conf/sfu.conf", "Configuration file")
