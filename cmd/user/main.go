...
import (
	...
	"github.com/doublemo/baa/cmd/user/server"
...
func ver() {
	fmt.Printf("user server version %s commitid %s builddate %s\n", version, commitid, builddate)
	os.Exit(0)
}
...
func main() {
	...
	fs.StringVar(&fp, "c", "conf/user.conf", "Configuration file")
	fs.StringVar(&fp, "config", "conf/user.conf", "Configuration file")
